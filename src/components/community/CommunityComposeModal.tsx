  ...
  return (
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-[56] max-h-[min(88vh,640px)] w-[min(92vw,400px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[1.35rem] bg-white p-5 shadow-2xl ring-2 ring-[#FFD2BF]/40">
        <p className="text-base font-extrabold text-slate-900">오늘의 순간 공유하기</p>
        <p className="mt-1 text-[11px] font-medium text-slate-500">
          방을 고르면 해당 토픽 피드에도 함께 올라가요. 이 기기에만 저장되는 데모예요.
        </p>

        <label className="mt-4 block text-[11px] font-extrabold text-slate-700">올릴 방</label>
        <select
          value={roomId}
          onChange={(e) => setRoomId(e.target.value)}
          className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-[13px] font-semibold text-slate-800 outline-none ring-1 ring-slate-100 focus:border-[#FFD2BF] focus:ring-2 focus:ring-[#FF853E]/20"
        >
        ...
        <label className="mt-3 flex cursor-pointer items-center gap-2 text-[13px] font-semibold text-slate-700">
          <input
            type="checkbox"
            checked={withPhoto}
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button
            type="button"
            onClick={submit}
            className="flex-1 rounded-full bg-[#FF853E] py-3 text-sm font-extrabold text-white shadow-md hover:brightness-[1.03]"
          >
            올리기
          </button>
        </div>
