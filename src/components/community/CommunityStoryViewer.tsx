    ...
    return (
      <div className="fixed inset-0 z-[80] flex flex-col bg-slate-900 text-white">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
          ...
          <button type="button" onClick={onClose} className="mt-6 rounded-full bg-white px-6 py-3 text-sm font-extrabold text-slate-900">
            돌아가기
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-black">
      <div className="flex shrink-0 items-center justify-between gap-2 px-4 py-3 pt-[max(0.5rem,env(safe-area-inset-top))] text-white">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-extrabold leading-snug line-clamp-2">{room.name}</p>
      ...
      <div className="flex shrink-0 justify-center gap-3 border-t border-white/10 bg-black/60 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        ...
        <button type="button" onClick={next} disabled={idx >= slides.length - 1} className="rounded-full bg-[#FF853E] px-6 py-2.5 text-sm font-extrabold text-white disabled:opacity-30">
          다음
        </button>
      </div>
    </div>
