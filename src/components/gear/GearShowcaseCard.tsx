    ...
    <>
      ...
      <div className="px-3 pb-3.5 pt-2">
        ...
        <p className="mt-2 flex flex-wrap items-baseline gap-x-1 text-[11px]">
          {instagramListing ? (
            <span className="font-extrabold text-fuchsia-600">Instagram에서 보기</span>
          ) : (
            <>
              <span className="font-extrabold text-[#FF853E]">{discount}%</span>
              <span className="font-bold text-slate-900">{product.price}원</span>
            </>
          )}
        </p>
        <p className="mt-1.5 text-[10px] text-slate-400">
          관심 <span className="font-semibold text-slate-500">{product.score}</span>
