        ...
        return (
          <div key={index} className="space-y-4 rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-extrabold text-slate-800">아이 {index + 1}</p>
              {children.length > 1 && (
                <button
                  type="button"
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {index === 0 ? (
                  <button
                    type="button"
                    onClick={onAddChild}
                    className="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-[#FFF1EA] text-lg font-extrabold text-[#FF853E] ring-1 ring-[#FFD2BF] hover:bg-[#ffe8dc]"
                    aria-label="아이 추가"
                  >
                    +
                  </button>
